...
class FormulaMainView(LabelFrame):
    ...
    def __init__(self, parent, text=''):  # gui parent not class parent
        ...
        self.variables_text.insert(END,
                                   'Enter variables and values:')  # prompt for users to edit and assign variables values
        self.variables_text.pack()

        self.btn_eval = Button(self, relief=RAISED, text='Evaluate Formula',
                               command=self._on_evaluate)  # initiates the _on_evaluate function to start a pipeline to evaluate and return answer
        self.btn_eval.pack()
    ...
    def add_new_formula(self, formula_text):  # the function that sets up formula text to be added stuff to
        ...
        self._update_rpn_textbox(
            self.formula.get_rpn())  # gets rpn and pipelines the editing of the textbox to display rpn
    ...
    def _get_user_values(self):  # get user values for variables
        ...
        if set(self.variables) != set(
                variables_dict.keys()):  # comparison to check if variables match the dictionary set
            self._update_answer_textbox(
                'Wrong set of variables are provided or this function is not implemented')  # error message if they do not match
            return False
    ...
    def _update_rpn_textbox(self, rpn_text):  # rpn textbox update
        ...
        # return
        show_text = ' '.join(show_text)  # joins the text from show text list into a string
    ...
    def _on_delete_formula(self):
        # print('delete')
        for subscriber in self.formula_delete_subscribers:  # uses the number of subscribers as a length for the loop
